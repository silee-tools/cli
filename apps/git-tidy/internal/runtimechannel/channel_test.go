...
func TestReleaseExecutableSelectsFixedReleaseForDevBuild(t *testing.T) {
	_, statePath, executable := runtimePaths(t, "git-tidy")
	writeFile(t, executable, "release", 0o755)
	writeFile(t, statePath, "channel=release\n", 0o644)

	got, err := ReleaseExecutable("dev", statePath, "git-tidy")
	if err != nil {
		t.Fatal(err)
	}
	if got != executable {
...
func TestReleaseExecutableRejectsUnknownChannel(t *testing.T) {
	_, statePath, _ := runtimePaths(t, "git-tidy")
	writeFile(t, statePath, "channel=unexpected\n", 0o644)

	_, err := ReleaseExecutable("dev", statePath, "git-tidy")
...
func TestReleaseExecutableIgnoresChannelForReleaseBuild(t *testing.T) {
	...
	got, err := ReleaseExecutable("1.2.3", statePath, "git-tidy")
	if err != nil {
		t.Fatal(err)
	}
	if got != "" {
...
func TestReleaseExecutableFindsStateAfterHomebrewIsInstalled(t *testing.T) {
	prefix, statePath, executable := runtimePaths(t, "git-tidy")
	writeFile(t, executable, "release", 0o755)
	writeFile(t, statePath, "channel=release\n", 0o644)
	...
	got, err := ReleaseExecutable("dev", "", "git-tidy")
	if err != nil {
		t.Fatal(err)
	}
	if got != executable {
