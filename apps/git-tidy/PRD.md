...
git-tidy 는 로컬 git 브랜치를 일괄 정리하는 zsh 플러그인이다. 보호 규칙으로 정리에서
제외할 브랜치를 정의하고, 보호되지 않은 나머지 브랜치를 정리 대상으로 삼는다. 작업이
끝나 더 둘 필요가 없는 잔여 브랜치를 한 번에 걷어내는 것이 목적이며, 정리해서는 안
되는 브랜치는 보호 규칙으로만 지킨다.
...
- git-tidy 는 보호 규칙으로 정리에서 제외할 브랜치를 정의하고, 보호되지 않은 나머지
  브랜치를 모두 정리 대상으로 삼는다. 즉 정리 대상을 일부만 골라내는 것이 아니라,
  보호 규칙이 정리 범위를 결정하는 유일한 수단이다.
