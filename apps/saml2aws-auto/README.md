...
기본적으로 `~/.saml2aws`의 `username` 값을 읽어 `totp "MS: <username>"`을 호출하고, `saml2aws login --force --skip-prompt --password="" --mfa-token=<코드>`를 실행한다. `SAML2AWS_USERNAME`이 이미 환경에 있으면 그 값을 우선한다.
