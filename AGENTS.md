...
- 현재 등록된 도구: jg, totp, saml2aws-auto, git-tidy. saml2aws-auto 는 `login`/`check`/`init zsh` 하위 명령과 zsh plugin 을 함께 제공한다. git-tidy 는 jg·totp 와 동일하게 Go 바이너리로 빌드되는 도구로, `apps/git-tidy/cmd/git-tidy/` 진입점과 `internal/` 패키지로 구성된다.
